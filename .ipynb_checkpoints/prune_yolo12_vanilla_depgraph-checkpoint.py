...
def postprocess_fix_split_modules(model, example_inputs=None, tp=None):
    """
    修复 C2f/A2C2f 等模块的 split 约束：
    Ultralytics 的某些模块 forward 里会：
        a, b = self.cv1(x).split((self.c, self.c), dim=1)

    剪枝后 cv1.out_channels 变了，但 self.c 还是旧值，就会报：
        split_with_sizes expects split_sizes to sum exactly to XXX, but got [c, c]

    这里不要再 build DependencyGraph（会触发前向 tracing，当前模型可能还没修好，会连锁崩）。
    只做一件事：让 self.c = cv1.out_channels // 2
    """
    fixed = 0
    for name, m in model.named_modules():
        if hasattr(m, "cv1") and hasattr(m, "c"):
            conv = unwrap_conv(getattr(m, "cv1"))
            if conv is None:
                continue
            outc = conv.out_channels
            if outc % 2 != 0:
                # outc 是奇数时，原版 split((c,c)) 无解
                # 这里先打印警告并跳过，避免再次触发前向错误
                print(f"[后处理][警告] {name}: cv1.out_channels={outc} 为奇数，split((c,c)) 无法成立，建议将该层加入 ignored_layers 防止被剪。")
                continue

            new_c = outc // 2
            if int(getattr(m, "c")) != int(new_c):
                setattr(m, "c", int(new_c))
                fixed += 1

    print(f"[后处理] split 模块修复完成，共修正 {fixed} 处")
    return fixed
...
def unwrap_conv(m):
    # 适配 Conv / Sequential / Ultralytics Conv
    if isinstance(m, torch.nn.Conv2d):
        return m
    if hasattr(m, "conv") and isinstance(m.conv, torch.nn.Conv2d):
        return m.conv
...
def get_ignored_layers(model):
    ignored = []
        ...
        # ✅ 直接把所有 cv1 加入 ignored
    for m in model.modules():
        if hasattr(m, "cv1"):
            ignored.append(m.cv1)

    # ========== 2️⃣ Detect head：最后输出 conv 不剪 ==========
    for m in model.modules():
        if m.__class__.__name__ == "Detect":
            for branch in list(m.cv2) + list(m.cv3):
                last = unwrap_conv(branch)
                if last is not None:
                    ignored.append(last)
    ...
    # ========== 4️⃣ Attention 类模块：qkv/proj/pe 等关键层不剪 ==========
    for m in model.modules():
        cls = m.__class__.__name__.lower()
        if ("attn" in cls) or ("attention" in cls):
            for attr in ("qkv", "proj", "pe", "q", "k", "v", "kv"):
                sub = getattr(m, attr, None)
                conv = unwrap_conv(sub) if sub is not None else None
                if conv is not None:
                    ignored.append(conv)

        # 有的模块是 m.attn 里面包了一层真正的 attention
        sub_attn = getattr(m, "attn", None)
        if sub_attn is not None:
            cls2 = sub_attn.__class__.__name__.lower()
            if ("attn" in cls2) or ("attention" in cls2):
                for attr in ("qkv", "proj", "pe", "q", "k", "v", "kv"):
                    sub = getattr(sub_attn, attr, None)
                    conv = unwrap_conv(sub) if sub is not None else None
                    if conv is not None:
                        ignored.append(conv)
...
def physical_pruning(args, tp):
    ...
    postprocess_fix_split_modules(model, example_inputs, tp)
